...
describe("IntegratedReaderView", () => {
  const mockArticle = {
    ...
    content:
      "<p>This is the article content with <strong>some formatting</strong>.</p>",
  ...
  describe("rendering", () => {
    ...
    it("should be hidden when isVisible is false", () => {
      ...
      // Vanilla Extractのクラス名をチェック（display: noneは隠しクラスで制御される）
      expect(readerView).toHaveClass(/integratedReaderContainerHidden/);
    });
  });
  ...
  describe("settings panel interaction", () => {
    ...
    it("should hide settings panel when close button is clicked", async () => {
      ...
      // SettingsPanelの隠しクラスをチェック
      expect(settingsPanel).toHaveClass("settings-panel-hidden");
    });
  });
  ...
  describe("settings persistence", () => {
    it("should apply settings changes to the reader view", async () => {
      ...
      expect(largeButton).toHaveClass("button-active");
    });
  });
  ...
  describe("toolbar", () => {
    it("should render toolbar with controls", () => {
      ...
      expect(
        screen.getByRole("button", { name: /close reader/i }),
      ).toBeInTheDocument();
    });
