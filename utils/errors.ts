 ...
 */
export const createReactUnmountError = (
  originalError: unknown
): ReaderViewError => {
...
export const createPopupContainerError = (): ReaderViewError => {
  return new ReaderViewError({
    ...
    message:
      'ポップアップコンテナの作成に失敗しました。bodyまたはdocumentElementが見つかりません。',
  });
};
 ...
 * スタイル設定保存失敗エラーを作成
 */
export const createStyleSaveError = (
  originalError: unknown
): ReaderViewError => {
  return new ReaderViewError({
 ...
 * スタイル設定読み込み失敗エラーを作成
 */
export const createStyleLoadError = (
  originalError: unknown
): ReaderViewError => {
  return new ReaderViewError({
